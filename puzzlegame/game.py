    ...
    while True:
        ...
        for event in pygame.event.get():#事件监听
            ...
            if event.type == KEYDOWN:#键盘事件
                if event.key == K_LEFT or event.key == ord('a'):#左
                    blackcell = moveleft(gameboard, blackcell)
                if event.key == K_RIGHT or event.key == ord('d'):#右
                    blackcell = moveright(gameboard, blackcell)
                if event.key == K_UP or event.key == ord('w'):#上
                    blackcell = moveup(gameboard, blackcell)
                if event.key == K_DOWN or event.key == ord('s'):#下
            ...
            if event.type == MOUSEBUTTONDOWN and event.button == 1:#鼠标事件
                ...
                if (index == blackcell - 1 or index == blackcell + 1 or index == blackcell - VHNUMS or index == blackcell + VHNUMS):#点击想要移动的图块（判断是否可以移动）
                        gameboard[blackcell], gameboard[index] = gameboard[index], gameboard[blackcell]
                        blackcell = index#移动白块位置
                        cnt = cnt+1
            if (isfinished(gameboard, blackcell)):
                gameboard[blackcell] = CELLNUMS - 1
                finish = True
            screen.fill(bg)#覆盖上白色背景
            # 图像分割
